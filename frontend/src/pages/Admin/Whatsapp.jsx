...
function Whatsapp() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const list = Array.isArray(response.data.offers)
        ...
        setSelectedId(
          (current) =>
            current ||
            list.find((offer) => offer.active)?._id ||
            list[0]?._id ||
            null,
        );
      } catch (error) {
        if (!cancelled)
          toast.error(
            error?.response?.data?.message || "Unable to load offers.",
          );
      } finally {
  ...
  const selectedOffer =
    offers.find((offer) => offer._id === selectedId) || null;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast.success(
        `"${selectedOffer.name}" queued for ${queued} customer${queued === 1 ? "" : "s"}.`,
      );
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Unable to queue WhatsApp campaign.",
      );
    } finally {
      setSending(false);
    }
    ...
    <div className="w-full max-w-[1920px] mx-auto space-y-6 lg:space-y-8 px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
      <div className="min-w-0">
        <h1 className="text-[clamp(1.5rem,1vw+1.1rem,1.875rem)] font-semibold flex flex-wrap items-center gap-2">
          <MessageCircle className="h-7 w-7 text-green-600" /> Send Offer on
          WhatsApp
        </h1>
        <p className="text-muted-foreground mt-1">
          Pick an offer, preview the message, and broadcast it to every customer
          on file.
        </p>
      </div>
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-primary/20 bg-primary/10 px-4 py-3 text-sm text-primary">
          ...
          <div>
            <div className="font-medium">
              Campaign has been queued successfully.
            </div>
            <div>
              &ldquo;{lastResult.offerName}&rdquo; was queued for{" "}
              {lastResult.queued} customer{lastResult.queued === 1 ? "" : "s"}{" "}
              and is being processed in the background. You can monitor delivery
              status and analytics from the TrdAI Dashboard.
            </div>
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="rounded-xl border border-input bg-muted/50 px-4 py-3 text-sm text-muted-foreground">
          Loading offers...
        </div>
      )}
      ...
      {offers.length > 0 && (
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="surface-card rounded-3xl border border-border bg-background p-5 shadow-sm flex flex-col">
            ...
            {selectedOffer ? (
              <>
                <div className="flex-1 rounded-2xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900 p-5">
                  <div className="max-w-[85%] rounded-2xl rounded-tl-sm border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 p-4 shadow-sm">
                    <div className="text-base font-semibold text-slate-900 dark:text-slate-100">
                      {selectedOffer.name}
                    </div>
                    <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300 whitespace-pre-wrap">
                      {buildOfferHighlight(selectedOffer)}
                    </p>
                  </div>
                </div>
                {!selectedOffer.active && (
                  <p className="mt-3 text-xs text-amber-600">
                    This offer is inactive — activate it from the Offers page
                    before sending.
                  </p>
                )}
                <Button
