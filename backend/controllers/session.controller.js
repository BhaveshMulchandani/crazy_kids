    ...
    if (!existingInvoice) {
      ...
      try {
        await Invoice.create({ invoiceNumber, session: session._id,
          ...
          payment: { status: session.paymentStatus || "pending", breakdown: session.paymentBreakdown || [], method: session.paymentMethod || "cash", amountPaid: Number(session.amountPaid || 0), pendingAmount: Math.max(calculation.grandTotal - Number(session.amountPaid || 0), 0) },
        });
      } catch (invoiceError) {
